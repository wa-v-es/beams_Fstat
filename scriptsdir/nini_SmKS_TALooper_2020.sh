...
# nini_SmKS_TALooper_2020.sh 1 Z AlSub_5 obs
#=============
#Codes needed:
#=============
...
if [ $gridtype == "test" ]; then
  slo_min=0.0; slo_inc=1; slo_max=15.0
  baz_min=-20; baz_inc=1; baz_max=20
elif [ $gridtype == "full" ]; then
  slo_min=0; slo_inc=0.25; slo_max=15.0
  baz_min=-50; baz_inc=.5; baz_max=50
fi

#Filtering: min, max, number of samples to use in F-trace window
if [ $datatype == "obs" ]; then
    fmin=.1;fmax=.5; delta=60
...
elif [ $region == "AlSub_5" ]; then
      grid_lat_min=61; grid_lat_max=65        #Range of lats to search and form subarrays # for subarray around ILAR
      grid_lon_min=-151; grid_lon_max=-146   #Range of lons to search and form subarrays

elif [ $region == "AlSub_sm_W" ]; then
      grid_lat_min=50; grid_lat_max=73        #Range of lats to search and form subarrays # for subarray around ILAR
      grid_lon_min=-180; grid_lon_max=-140
...
echo nini_SmKS_Vespapack_2020.sh $fmin $fmax $comp $delta $number_min $eventdate $basestation $basestation_lat $basestation_lon $grid_lat $grid_lon $master_results_list $grid_num $slo_min $slo_inc $slo_max $baz_min $baz_inc $baz_max $phase $phase_taup $DO_CIRC $DO_ELEV $elev_vel $DO_RADPAT
echo RUNNING
nini_SmKS_Vespapack_2020.sh $fmin $fmax $comp $delta $number_min $eventdate $basestation $basestation_lat $basestation_lon $grid_lat $grid_lon $master_results_list $grid_num $slo_min $slo_inc $slo_max $baz_min $baz_inc $baz_max $phase $phase_taup $DO_CIRC $DO_ELEV $elev_vel $DO_RADPAT
#TO REMOVE
