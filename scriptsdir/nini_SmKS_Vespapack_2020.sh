...
xf_slow_xyz_both=$temps"TEMP_XF_SLOW_XYZ_both_$$"
beam_slow_xyz_both=$temps"TEMP_BEAM_SLOW_XYZ_both_$$"
xf_baz_xyz_both=$temps"TEMP_XF_BAZ_XYZ_both_$$"
beam_baz_xyz_both=$temps"TEMP_BEAM_BAZ_XYZ_both_$$"
...
taup time -h $evdep --deg $dist --ph p,s,P,PP,PPP,PPPP,PPPPP,pP,pPP,pPPP,pPPPP,pPPPPP,Pdiff,pPdiff,P^410P,P^660P,PKiKP,SKS,PKS,sSKiKS,sSKIKS,SKIKS,SKiKS,Sdiff,SKKS,SKKKS,SKKKKS,ScS,sScS,S,sS,sP,sPdiff,SS,SSS,SSSS,SSSSS,sSS,sSSS,sSSSS,sSSSSS,sPKS,pPKS,sSKS,sSKKS,sSKKKS,sSKKKKS,PcS,pPcS,sPcS --mod $model > $temps"PHASE_TIMES_TEMP_$$"
#cat $temps"PHASE_TIMES_TEMP_$$"

p_wave=(`awk '/ p / {print $4, $5, "p"}' $temps"PHASE_TIMES_TEMP_$$" | awk 'NR==2'`)
s_wave=(`awk '/ s / {print $4, $5, "s"}' $temps"PHASE_TIMES_TEMP_$$"`)
...
${p_wave[0]} ${p_wave[1]} 0 ${p_wave[2]}
${s_wave[0]} ${s_wave[1]} 0 ${s_wave[2]}
EOF
...
#--------------------
#here such that time if P arrival + 50 sec

# if [ -z ${Pwave[0]} ]; then
# cutmin=`echo ${phasePRED[0]} | awk '{print $1-250}'`
# else
cutmin=`echo ${p_wave[0]} | awk '{print $1-50}'`
cutmin=0
echo p_wave ${p_wave[0]}
# fi
# cutmin=`echo ${phasePRED[0]} | awk '{print $1-300}'`
cutmax=`echo ${phasePRED[0]} | awk '{print $1+200}'`
cutmax=200
echo phasePRED ${phasePRED[0]}
...
\rm -f $xf_slow_xyz_both $beam_slow_xyz_both $xf_baz_xyz_both $beam_baz_xyz_both

touch $xf_slow_xyz $beam_slow_xyz $xf_baz_xyz $beam_baz_xyz $xf_slow_grid $beam_slow_grid $xf_baz_grid $beam_baz_grid
touch $xf_slow_xyz_both $beam_slow_xyz_both $xf_baz_xyz_both $beam_baz_xyz_both
...
sort -nr -k4 $FINALMAXFILE | awk '!seen[$1$3]++' | awk '{printf "%12.9f %12.9f %14.12f %14.12f %12.9f %12.9f\n", $3, $1, $4, $5, $2, $6}' > $OUTFILE_BAZ
#FOR SLOW- Remove duplicated Slow and Time
sort -nr -k4 $FINALMAXFILE | awk '!seen[$2$3]++' | awk '{printf "%12.9f %12.9f %14.12f %14.12f %12.9f %12.9f\n", $3, $2, $4, $5, $1, $6}' > $OUTFILE_SLOW
...
awk '{print $1, $2, $3, $5, $6}' $OUTFILE_SLOW > $xf_slow_xyz_both
awk '{print $1, $2, $4, $5, $6}' $OUTFILE_SLOW > $beam_slow_xyz_both
awk '{print $1, $2, $3, $5, $6}' $OUTFILE_BAZ > $xf_baz_xyz_both
awk '{print $1, $2, $4, $5, $6}' $OUTFILE_BAZ > $beam_baz_xyz_both
...
#PLOT setup
plotcutmin=`echo $cutmin | awk '{print $1+2}'`
plotcutmax=`echo $cutmax | awk '{print $1-2}'`
...
awk -v tmin=${filttime_minmax[0]} -v tmax=${filttime_minmax[1]} '$1>(tmin+10) && $1<(tmax-10) {print $1, '$dist', ($3/'$vmax')}' filt_max.xy | gmt pswiggle $sac_range $sac_frame -Z$sac_scale_y_dbl -W0.04c,purple -O -K >> $outps
echo 10.75 $dist BEAM | gmt pstext -F+jLM+a0+f7,1,purple $sac_range_text $sac_frame -N -O -K >> $outps
...
#
mv $xf_slow_xyz_both xyz_folder/xf_slow_xyz"_"$grid_num"_"$eventdate"_"$fmin"_"$fmax.xyz
mv $beam_slow_xyz_both xyz_folder/beam_slow_xyz"_"$grid_num"_"$eventdate"_"$fmin"_"$fmax.xyz
mv $xf_baz_xyz_both xyz_folder/xf_baz_xyz"_"$grid_num"_"$eventdate"_"$fmin"_"$fmax.xyz
mv $beam_baz_xyz_both xyz_folder/beam_baz_xyz"_"$grid_num"_"$eventdate"_"$fmin"_"$fmax.xyz
