...
! Local variables
REAL (kind=8), PARAMETER  :: e = .918938533204673_dp
REAL (kind=8)             :: a, b, c, h, u, v, w, z
INTEGER               :: i, n
REAL (kind=8), PARAMETER  :: half = 0.5_dp, one = 1.0_dp, two = 2.0_dp,  &
                         eight = 8.0_dp
!--------------------------
...
IMPLICIT NONE
INTEGER, PARAMETER    :: dp = SELECTED_REAL_KIND(15, 100)
REAL (kind=8), INTENT(IN) :: x, p, q, beta
REAL (kind=8)             :: fn_val

! Local variables
LOGICAL               :: indx
INTEGER               :: ns
REAL (kind=8)             :: psq, cx, xx, pp, qq, term, ai, rx, temp

!   Define accuracy and initialise

REAL (kind=8), PARAMETER  :: zero = 0.0_dp, one = 1.0_dp, acu = 1.0E-10_dp
...
IF(x < zero .OR. x > one) THEN
  WRITE(*, *) 'x',x,'AS63: Argument x outside range (0, 1)'
  RETURN
END IF
...
!     Calculate result
!If EXPONENT will be less then -50 then print RESULT as 0 to prevent UNDERFLOW - MODIFIED BY DAN FROST (26.10.16)
5 if ((pp*LOG(xx) + (qq-one)*LOG(cx) - beta) < (-50) ) then
!write(*,*) "exp",(pp*LOG(xx) + (qq-one)*LOG(cx) - beta)
fn_val=0
!Else calculate RESULT - MODIFIED BY DAN FROST (26.10.16)
else
